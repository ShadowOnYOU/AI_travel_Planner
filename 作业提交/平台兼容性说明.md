# ⚠️ 平台兼容性说明

## 🏗️ Docker 镜像平台支持

当前 Docker 镜像是基于 `linux/amd64` 平台构建的，主要支持：

### ✅ 支持的平台
- **Intel/AMD x86_64 处理器**的 Linux 系统
- **Intel 处理器**的 Windows 系统（通过 WSL2）  
- **Intel 处理器**的 macOS 系统

### ⚠️ 可能需要额外配置的平台
- **Apple Silicon (M1/M2)** 的 macOS 系统
- **ARM64** 架构的 Linux 系统

## 🛠️ Apple Silicon (M1/M2) Mac 解决方案

如果您使用的是 Apple Silicon Mac，请使用以下方法：

### 方法一：强制使用 x86_64 平台（推荐）

```bash
# 强制拉取 x86_64 版本镜像
docker pull --platform linux/amd64 crpi-ttmopfgqdmld0jn5.cn-hangzhou.personal.cr.aliyuncs.com/ai_by_sgh/ai-travel-planner:latest

# 运行容器（会通过 Rosetta 2 转换）
docker run -d \
  --name ai-travel-planner \
  -p 3000:3000 \
  --platform linux/amd64 \
  crpi-ttmopfgqdmld0jn5.cn-hangzhou.personal.cr.aliyuncs.com/ai_by_sgh/ai-travel-planner:latest
```

### 方法二：本地构建 ARM64 版本

```bash
# 1. 克隆项目
git clone https://github.com/ShadowOnYOU/AI_travel_Planner.git
cd AI_travel_Planner

# 2. 本地构建适合 ARM64 的镜像
docker build --platform linux/arm64 -t ai-travel-planner:arm64 .

# 3. 运行本地构建的镜像
docker run -d \
  --name ai-travel-planner \
  -p 3000:3000 \
  ai-travel-planner:arm64
```

### 方法三：使用 Docker Desktop 的 Rosetta 2

1. 在 Docker Desktop 设置中启用 "Use Rosetta for x86/amd64 emulation on Apple Silicon"
2. 重启 Docker Desktop
3. 正常运行镜像（会自动转换）

## 🐧 Linux ARM64 系统解决方案

对于 ARM64 架构的 Linux 系统：

```bash
# 本地构建 ARM64 版本
git clone https://github.com/ShadowOnYOU/AI_travel_Planner.git
cd AI_travel_Planner
docker build --platform linux/arm64 -t ai-travel-planner:arm64 .
docker run -d --name ai-travel-planner -p 3000:3000 ai-travel-planner:arm64
```

## 🪟 Windows 系统说明

Windows 用户需要：
1. 安装 Docker Desktop for Windows
2. 启用 WSL2 后端
3. 确保使用 Linux 容器模式

```cmd
# Windows 命令提示符或 PowerShell
docker run -d --name ai-travel-planner -p 3000:3000 crpi-ttmopfgqdmld0jn5.cn-hangzhou.personal.cr.aliyuncs.com/ai_by_sgh/ai-travel-planner:latest
```

## 🔍 检查系统架构

使用以下命令检查您的系统架构：

```bash
# 检查 Docker 系统信息
docker version

# 检查系统架构
uname -m

# 检查 Docker 支持的平台
docker buildx ls
```

常见输出：
- `x86_64` 或 `amd64`: Intel/AMD 64位处理器
- `arm64` 或 `aarch64`: ARM 64位处理器（包括 Apple Silicon）
- `armv7l`: ARM 32位处理器

## 🚀 通用运行方案

### 智能检测脚本

创建一个能自动检测架构的运行脚本：

```bash
#!/bin/bash

# 检测系统架构
ARCH=$(uname -m)
echo "检测到系统架构: $ARCH"

if [[ "$ARCH" == "arm64" ]] || [[ "$ARCH" == "aarch64" ]]; then
    echo "检测到 ARM64 架构，将使用平台兼容模式..."
    docker run -d \
      --name ai-travel-planner \
      -p 3000:3000 \
      --platform linux/amd64 \
      crpi-ttmopfgqdmld0jn5.cn-hangzhou.personal.cr.aliyuncs.com/ai_by_sgh/ai-travel-planner:latest
else
    echo "检测到 x86_64 架构，直接运行镜像..."
    docker run -d \
      --name ai-travel-planner \
      -p 3000:3000 \
      crpi-ttmopfgqdmld0jn5.cn-hangzhou.personal.cr.aliyuncs.com/ai_by_sgh/ai-travel-planner:latest
fi

echo "启动完成！访问 http://localhost:3000"
```

## 📋 故障排查

### 常见错误

1. **"no matching manifest for linux/arm64/v8"**
   - 原因：镜像不支持当前架构
   - 解决：使用 `--platform linux/amd64` 参数

2. **容器运行缓慢**
   - 原因：架构转换开销
   - 解决：本地构建对应架构的镜像

3. **拉取失败**
   - 原因：网络问题或镜像仓库访问限制
   - 解决：使用本地构建方式

### 性能对比

| 平台组合 | 性能表现 | 推荐程度 |
|---------|---------|---------|
| x86_64 → x86_64 镜像 | 100% | ⭐⭐⭐⭐⭐ |
| ARM64 → ARM64 镜像 | 100% | ⭐⭐⭐⭐⭐ |
| ARM64 → x86_64 镜像 | 70-80% | ⭐⭐⭐ |

## 💡 最佳实践建议

1. **对于 x86_64 系统**: 直接使用提供的镜像
2. **对于 ARM64 系统**: 优先尝试 `--platform` 参数，如性能不足则本地构建
3. **对于生产环境**: 建议构建对应架构的镜像以获得最佳性能

---

**🎯 快速测试命令（自动适配架构）**:
```bash
docker run --rm --platform linux/amd64 -p 3000:3000 crpi-ttmopfgqdmld0jn5.cn-hangzhou.personal.cr.aliyuncs.com/ai_by_sgh/ai-travel-planner:latest
```